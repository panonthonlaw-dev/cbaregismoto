...
def create_pdf_tra(vals, img_url1, img_url2, face_url=None, printed_by="N/A"):
    ...
    c.setFont(fn, 16); c.drawString(60, height - 110, f"ชื่อ: {vals[1]}"); c.drawString(350, height - 110, f"ยี่ห้อ: {vals[4]}")
    c.drawString(60, height - 130, f"ID: {vals[2]}"); c.drawString(350, height - 130, f"สี: {vals[5]}")
    score = str(vals[13]) if str(vals[13]).isdigit() else "100"
    c.setFont(fb, 18); c.drawString(60, height - 180, f"คะแนนคงเหลือ: {score} คะแนน")
    ...
    c.setFont(fb, 16); c.drawString(60, height - 410, "📝 ประวัติ:"); c.setFont(fn, 14)
    raw_h = str(vals[12]).strip(); h_text = raw_h if raw_h and raw_h.lower() != "nan" else "ไม่พบประวัติ"
    to = c.beginText(70, height - 430); to.setLeading(18)
    for line in h_text.split('\n'):
        for wl in textwrap.wrap(line, width=80): to.textLine(wl)
    c.drawText(to)
    c.setFont(fn, 10); c.setFillColorRGB(0.5, 0.5, 0.5)
    c.drawRightString(width - 50, 30, f"ผู้พิมพ์: {printed_by} | {datetime.now(thai_tz).strftime('%d/%m/%Y %H:%M')}")
    c.save(); buffer.seek(0); return buffer

# ✅ 6. MODULE: TRAFFIC (รวมทุกระบบ)
def traffic_module():
    ...
    if st.session_state.traffic_page == 'main':
        ...
        if st.session_state.df_tra is not None:
            ...
            m2.metric("มีใบขับขี่", f"{len(df[df['C7'] == '✅ มี'])} คน")
            m3.metric("ภาษีปกติ", f"{len(df[df['C8'].str.contains('ปกติ|✅', na=False)])} คัน")
                    ...
                    with st.expander(f"📌 {v[6]} | {v[1]} (แต้ม: {sc})"):
                        ...
                        if st.session_state.officer_role in ["admin", "super_admin"]:
                            ...
                            if st.session_state.officer_role == "super_admin":
                                if c_edit.button("✏️ แก้ไขข้อมูลนักเรียน", key=f"ed_{i}", use_container_width=True):
                                    st.session_state.edit_data = v; st.session_state.traffic_page = 'edit'; st.rerun()

                            with st.form(key=f"sc_form_{i}"):
                                pts = st.number_input("แต้ม", 1, 50, 5); note = st.text_area("หมายเหตุ")
                                b1, b2 = st.columns(2)
        ...
        if st.session_state.officer_role == "super_admin":
            ...
            with st.expander("⚙️ เมนูเลื่อนชั้นเรียน (Super Admin)"):
                up_p = st.text_input("รหัสยืนยัน", type="password", key="p_p")
                if st.button("🚀 ตกลงเลื่อนชั้นทั้งโรงเรียน") and up_p == UPGRADE_PASSWORD:
                    try:
        ...
        with st.form("edit_form_student"):
            nm = st.text_input("ชื่อ-นามสกุล", v[1]); cl = st.text_input("ระดับชั้น", v[3])
            br = st.selectbox("ยี่ห้อ", ["Honda", "Yamaha", "Suzuki", "GPX", "Kawasaki", "อื่นๆ"], index=0); co = st.text_input("สีรถ", v[5]); pl = st.text_input("ทะเบียน", v[6])
            lc = st.radio("ใบขับขี่", ["✅ มี", "❌ ไม่มี"], horizontal=True); tx = st.radio("ภาษี", ["✅ ปกติ", "❌ ขาด"], horizontal=True); hel = st.radio("หมวก", ["✅ มี", "❌ ไม่มี"], horizontal=True)
            u_f = st.file_uploader("เปลี่ยนรูปหน้าเจ้าของ"); u_b = st.file_uploader("เปลี่ยนรูปหลังรถ"); u_s = st.file_uploader("เปลี่ยนรูปข้างรถ")
            if st.form_submit_button("💾 บันทึกการแก้ไข", type="primary", use_container_width=True):
                ...
                if u_f: l1 = upload_to_drive(u_f, f"{v[2]}_F_edit.jpg")
                if u_b: l2 = upload_to_drive(u_b, f"{v[2]}_B_edit.jpg")
                if u_s: l3 = upload_to_drive(u_s, f"{v[2]}_S_edit.jpg")
                # อัปเดตข้อมูล B ถึง L (ยกเว้น ID) และ N, O
                sheet.update(range_name=f'B{cell.row}:L{cell.row}', values=[[nm, v[2], cl, br, co, pl, lc, tx, hel, l2, l3]])
                sheet.update_cell(cell.row, 15, l1)
                st.success("แก้ไขข้อมูลเรียบร้อย!"); st.session_state.df_tra = None; st.session_state.traffic_page = 'main'; st.rerun()
...
if st.session_state['page'] == 'student':
    ...
    with st.form("reg_form"):
        ...
        rm = sc4.text_input("ห้อง (เช่น 0-13)")
        pin = st.text_input("ตั้ง PIN 6 หลัก", type="password", max_chars=6)
        sc5, sc6 = st.columns(2)
        br = st.selectbox("ยี่ห้อรถ", ["Honda", "Yamaha", "Suzuki", "GPX", "Kawasaki", "อื่นๆ"])
        co = st.text_input("สีรถ"); pl = st.text_input("ทะเบียนรถ")
        ...
        if st.form_submit_button("ส่งข้อมูลลงทะเบียน", type="primary", use_container_width=True):
            if fname and sid and p1 and p2 and p3:
                try:
                    ...
                    if l1 and l2 and l3:
                        new_d = [datetime.now().strftime('%d/%m/%Y %H:%M'), f"{pre}{fname}", str(sid), f"{lv}/{rm}", br, co, pl, ls, ts, hs, l2, l3, "", "100", l1, str(pin)]
                        sheet.append_row(new_d); st.success("สำเร็จ!"); st.balloons(); time.sleep(1); st.rerun()
                except Exception as e: st.error(f"Error: {e}")
            else: st.error("❌ กรุณากรอกข้อมูลและอัปโหลดรูปภาพให้ครบ 3 รูป")
    st.divider()
            ...
            <div class="atm-card">
                ...
                <div style="display: flex; gap: 20px; margin-top: 15px;">
                    ...
                    <div style="flex: 1; color: #1e293b; line-height: 1.6;">
                        ...
                        <div style="font-size: 0.9rem;">🆔 {v[2]} | 🏍️ {v[6]} | 📚 {v[3]}</div>
                    </div>
                </div>
                <div style="position: absolute; bottom: 15px; right: 20px; text-align: right;">
                    <div style="font-size: 0.8rem; color: #64748b; font-weight: bold;">แต้มวินัยจราจร</div>
